...
class EntertainmentBedroomAll extends StatefulWidget {
  const EntertainmentBedroomAll({super.key, title});

  @override
  State<EntertainmentBedroomAll> createState() => _EntertainmentBedroomAllState();
}

class _EntertainmentBedroomAllState extends State<EntertainmentBedroomAll> with TickerProviderStateMixin {
  String title = '';
  ...
  double getMenuWidth() {
    switch (selectedSource) {
      case 'globalEntertainmentCamera6':
        return 0.3; // 30% pro kamery (málo tlačítek)
      case 'globalEntertainmentMap':
  ...
  String getTabText() {
    ...
    if (selectedSource == 'globalEntertainmentMap') {
      return 'MAP CONTROLS';
    }
  ...
  List<List<GenericSelection>> _getPresetsRowsForGroup(List<dynamic> menuItems) {
    if (selectedLightGroup == null) return [];

    switch (selectedLightGroup) {
      case 'a':
        return [];
      default:
        return [
  ...
  Widget _buildGlobalControls(CustomTheme myTheme) {
    final menuItems = configItems['lounge-lights'] as List<dynamic>;
    String menuPrefix = 'lounge_';
    ...
    GenericSelection allLightsDim = menuItems.firstWhere((element) => element.id == 'allLightsDim') as GenericSelection;
    GenericSelection allLightsMid = menuItems.firstWhere((element) => element.id == 'allLightsMid') as GenericSelection;
  ...
  Widget _buildGroupControls(CustomTheme myTheme) {
    List<dynamic> menuItems = configItems['global-entertainment'] as List<dynamic>;
    ...
    GenericSelection globalEntertainmentHDMILounge = menuItems.firstWhere((element) => element.id == 'globalEntertainmentHDMIBedroom') as GenericSelection;
    GenericSelection globalEntertainmentHDMIGlobal = menuItems.firstWhere((element) => element.id == 'globalEntertainmentHDMIGlobal') as GenericSelection;
    GenericSelection globalEntertainmentHDMIAVOD = menuItems.firstWhere((element) => element.id == 'globalEntertainmentHDMIAVOD') as GenericSelection;
    GenericSelection globalEntertainmentCamera1 = menuItems.firstWhere((element) => element.id == 'globalEntertainmentCamera1') as GenericSelection;
    GenericSelection globalEntertainmentCamera2 = menuItems.firstWhere((element) => element.id == 'globalEntertainmentCamera2') as GenericSelection;
    GenericSelection globalEntertainmentCamera3 = menuItems.firstWhere((element) => element.id == 'globalEntertainmentCamera3') as GenericSelection;
    GenericSelection globalEntertainmentCamera4 = menuItems.firstWhere((element) => element.id == 'globalEntertainmentCamera4') as GenericSelection;
    GenericSelection globalEntertainmentCamera5 = menuItems.firstWhere((element) => element.id == 'globalEntertainmentCamera5') as GenericSelection;
    GenericSelection globalEntertainmentCamera6 = menuItems.firstWhere((element) => element.id == 'globalEntertainmentCamera6') as GenericSelection;
  ...
  Widget _buildDynamicContent(CustomTheme myTheme) {
    // Camera 6 ovládání
    if (selectedSource == 'globalEntertainmentCamera6') {
  ...
  Widget _buildMonitorBrightnessControl(CustomTheme myTheme) {
    ...
    BargraphModel monitorBrightness = menuItems.firstWhere(
      (element) => element.id == 'monitorBrightness', // ⭐ ID z config
    ) as BargraphModel;
  ...
  Widget _buildJoystick(CustomTheme myTheme) {
    return Container(
      ...
      child: Joystick(
        ...
        listener: (details) {
          print('Joystick: x=${details.x}, y=${details.y}');
          // TODO: Implementuj ovládání mapy
        },
        base: JoystickBase(
...
// Helper class pro skupiny světel
class LightGroup {
  final String id;
