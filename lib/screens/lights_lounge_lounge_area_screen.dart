...
import 'lights_lounge_lounge_accent_panel.dart';
import 'lights_lounge_lounge_ceiling_panel.dart';
import 'lights_lounge_lounge_dnwash_panel.dart';
import 'lights_lounge_lounge_dome_panel.dart';
import 'lights_lounge_lounge_toekick_panel.dart';
import 'lights_lounge_lounge_upwash_panel.dart';

class LightsLoungeLoungeAreaScreen extends StatefulWidget {
  final RoomType roomType;

  const LightsLoungeLoungeAreaScreen({super.key, required this.roomType});

  @override
  State<LightsLoungeLoungeAreaScreen> createState() => _LightsLoungeLoungeAreaScreenState();
}

class _LightsLoungeLoungeAreaScreenState extends State<LightsLoungeLoungeAreaScreen> {
  ...
  static const List<TabSpec> _groupTabs = [
    TabSpec(id: 'ceiling', label: 'CEILING', flex: 20, iconSize: 60),
    TabSpec(id: 'dome', label: 'DOME', flex: 20, iconSize: 60),
    TabSpec(id: 'upwash', label: 'UPWASH', flex: 20, iconSize: 60),
    TabSpec(id: 'dnwash', label: 'DNWASH', flex: 20, iconSize: 60),
    TabSpec(id: 'toekick', label: 'TOEKICK', flex: 20, iconSize: 60),
    TabSpec(id: 'accent', label: 'ACCENT', flex: 20, iconSize: 60),
  ];
  ...
  Widget _buildRightPanel(CustomTheme myTheme) {
    final Map<String, WidgetBuilder> panels = {
      'ceiling': (_) => const LightsLoungeLoungeCeilingPanel(),
      'dome': (_) => const LightsLoungeLoungeDomePanel(),
      'upwash': (_) => const LightsLoungeLoungeUpwashPanel(),
      'dnwash': (_) => const LightsLoungeLoungeDnwashPanel(),
      'toekick': (_) => const LightsLoungeLoungeToekickPanel(),
      'accent': (_) => const LightsLoungeLoungeAccentPanel(),
    };
  ...
  Widget _buildDirLightsPanel(
    ...
    return Container(
      width: 490 + 240, // ⭐ přidáno místo pro tabs (nebo nech jen constraints a Row si to vezme)
            ...
            Expanded(
              child: Stack(
                children: [
                  ...
                  if (_imageSize != null) ...directionLights.map(buildDirLight),
                ],
              ),
            ),
  ...
  Map<String, GenericSelection> _buildTabPowerItems() {
    final candidates = <String, GenericSelection?>{
      'ceiling': _findGenericSelection('lights_lounge_ceiling', 'lounge_ceiling_power'),
      'dome': _findGenericSelection('lights_lounge_dome', 'lounge_dome_indication_power'),
      'upwash': _findGenericSelection('lights_lounge_upwash', 'lounge_upwash_indication_power'),
      'dnwash': _findGenericSelection('lights_lounge_dnwash', 'lounge_dnwash_indication_power'),
      'toekick': _findGenericSelection('lights_lounge_toekick', 'lounge_toekick_indication_power'),
      'accent': _findGenericSelection('lights_lounge_accent', 'lounge_accent_indication_power'),
    };
  ...
  Widget _buildGlobalControls(CustomTheme myTheme) {
    final menuItems = configItems['lights_lounge_all'] as List<dynamic>;
  ...
  Widget _buildSideMenuContent(CustomTheme myTheme) {
    final menuItems = configItems['lights_lounge_presets'] as List<dynamic>;
