...
import 'lights_aft_lav_rh_accent_panel.dart';
import 'lights_aft_lav_rh_ceiling_panel.dart';
import 'lights_aft_lav_rh_dnwash_panel.dart';
import 'lights_aft_lav_rh_toekick_panel.dart';

class LightsAftLavRhAreaScreen extends StatefulWidget {
  const LightsAftLavRhAreaScreen({super.key});

  @override
  State<LightsAftLavRhAreaScreen> createState() => _LightsAftLavRhAreaScreenState();
}

class _LightsAftLavRhAreaScreenState extends State<LightsAftLavRhAreaScreen> {
  ...
  Widget _buildRightPanel(CustomTheme myTheme) {
    final Map<String, WidgetBuilder> panels = {
      'ceiling': (_) => const LightsAftLavRhCeilingPanel(),
      'dnwash': (_) => const LightsAftLavRhDnwashPanel(),
      'toekick': (_) => const LightsAftLavRhToekickPanel(),
      'accent': (_) => const LightsAftLavRhAccentPanel(),
    };
  ...
  Widget _buildDirLightsPanel(
    ...
    Widget buildDirLight(GenericSelection item, DirLightLayoutRel rel) {
      final size = _imageSize;
      if (size == null) return const SizedBox.shrink();

      final left = size.width * rel.x - (rel.width / 2);
      final top = size.height * rel.y - (rel.height / 2);
          ...
          child: FittedBox(
            ...
            child: SizedBox(
              ...
              child: GenericSelectionWidget(
                ...
                height: rel.height,
                width: rel.width,
                imageSize: [rel.width, rel.height],
  ...
  Map<String, GenericSelection> _buildTabPowerItems() {
    final candidates = <String, GenericSelection?>{
      'ceiling': _findGenericSelection('lights_aft_lav_rh_ceiling', 'aft_lav_rh_ceiling_power'),
      'dnwash': _findGenericSelection('lights_aft_lav_rh_dnwash', 'aft_lav_rh_dnwash_indication_power'),
      'toekick': _findGenericSelection('lights_aft_lav_rh_toekick', 'aft_lav_rh_toekick_indication_power'),
      'accent': _findGenericSelection('lights_aft_lav_rh_accent', 'aft_lav_rh_accent_indication_power'),
    };
  ...
  Widget _buildGlobalControls(CustomTheme myTheme) {
     final menuItems = configItems['lights_aft_lav_rh_all'] as List<dynamic>;
    String menuPrefix = 'aft_lav_rh_';
  ...
  Widget _buildSideMenuContent(CustomTheme myTheme) {
    final menuItems = configItems['lights_aft_lav_rh_presets'] as List<dynamic>;
