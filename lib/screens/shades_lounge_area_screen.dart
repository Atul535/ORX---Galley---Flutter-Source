...
class ShadesLoungeAreaScreen extends StatefulWidget {
  const ShadesLoungeAreaScreen({super.key});

  @override
  State<ShadesLoungeAreaScreen> createState() => _ShadesLoungeAreaScreenState();
}

class _ShadesLoungeAreaScreenState extends State<ShadesLoungeAreaScreen> {
  ...
  Widget _buildRightPanel(CustomTheme myTheme) {
    final Map<String, WidgetBuilder> panels = {
      'left': (_) => const ShadesLoungeLeftPanel(),
      'right': (_) => const ShadesLoungeRightPanel(),
      'all': (_) => const ShadesLoungeAllPanel(),
    };
  ...
  ({List<GenericSelection> top, List<GenericSelection> bottom}) _splitDirLightsForShades(List<GenericSelection> all) {
    ...
    final bottom = all.skip(8).take(7).toList();
