...
class EntertainmentBusinessClassRh extends StatefulWidget {
  const EntertainmentBusinessClassRh({super.key, title});

  @override
  State<EntertainmentBusinessClassRh> createState() => _EntertainmentBusinessClassRhState();
}

class _EntertainmentBusinessClassRhState extends State<EntertainmentBusinessClassRh> with TickerProviderStateMixin {
  ...
  Widget _buildMonitorBrightnessControl(CustomTheme myTheme) {
    final menuItems = configItems['business_video_rh'] as List<dynamic>;
    final BargraphModel monitorBrightness = menuItems.firstWhere((e) => e.id == 'business_brightnessRh') as BargraphModel;
  ...
  Widget _buildMapControls(CustomTheme myTheme) {
    ...
    final zoomIn = menuItems.firstWhere((e) => e.id == 'mapZoomIn') as GenericSelection;
    final zoomOut = menuItems.firstWhere((e) => e.id == 'mapZoomOut') as GenericSelection;
    ...
    final GenericSelection monitorPwr = configItems['business_video_rh']?.firstWhere((e) => e.id == 'business_MonitorPwrRh') as GenericSelection;
          ...
          Column(
            ...
            children: [
              Selector<CurrentStateProvider, int>(
                selector: (context, p) => p.getCurrentState(monitorPwr.id.toString()),
                builder: (context, currStateValue, child) {
                  return GenericSelectionWidget(
                    id: monitorPwr.id,
                    title: monitorPwr.title,
                    icons: monitorPwr.icons,
                    iconSize: monitorPwr.iconSize,
                    isMomentary: monitorPwr.isMomentary,
                    onStateCallBack: () {},
                    offStateCallBack: () {},
                    height: monitorPwr.height,
                    width: monitorPwr.width,
                    textIconSpacing: 10,
                    states: monitorPwr.states,
  ...
  Widget _buildJoystick(CustomTheme myTheme) {
    return SizedBox(
      ...
      child: Joystick(
        ...
        listener: (details) => print('Joystick: x=${details.x}, y=${details.y}'),
        base: JoystickBase(
