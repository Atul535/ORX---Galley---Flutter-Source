...
import 'shades_dining_all_panel.dart';
import 'shades_dining_left_panel.dart';
import 'shades_dining_right_panel.dart';
import 'shades_lounge_all_panel.dart';
import 'shades_lounge_left_panel.dart';
import 'shades_lounge_right_panel.dart';

class ShadesDiningAreaScreen extends StatefulWidget {
  const ShadesDiningAreaScreen({super.key});

  @override
  State<ShadesDiningAreaScreen> createState() => _ShadesDiningAreaScreenState();
}

class _ShadesDiningAreaScreenState extends State<ShadesDiningAreaScreen> {
  ...
  Widget _buildRightPanel(CustomTheme myTheme) {
    final Map<String, WidgetBuilder> panels = {
      'left': (_) => const ShadesDiningLeftPanel(),
      'right': (_) => const ShadesDiningRightPanel(),
      'all': (_) => const ShadesDiningAllPanel(),
    };
  ...
  ({List<GenericSelection> top, List<GenericSelection> bottom}) _splitDirLightsForShades(List<GenericSelection> all) {
    ...
    final top = all.take(9).toList();
    final bottom = all.skip(9).take(9).toList();
