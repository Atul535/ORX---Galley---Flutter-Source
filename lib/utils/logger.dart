...
void __log(
  ...
  // Prepend the formatted time to the message:
  String timeStampedMessage = "[$formattedTime] $message";

  // Log the message to DevTools if option has been set
  if (_logger.logDevToolsOnly) {
    dart.log(
      timeStampedMessage,
      time: null,
  ...
  flutter.debugPrint("${_color[level]!}$headerFull$timeStampedMessage\x1B[0m"
      // _color[level]! + _headerFull + timeStampedMessage + '\x1B[0m'
      );
  if (object != null) {
    // If log call passed an object to print:
    try {
      // Try to print object.
      flutter.debugPrint("${_color[level]!}${object.toString}\x1B[0m"
          // _color[level]! + object.toString() + '\x1B[0m'
          );
    } catch (error) {
      // Otherwise, print error.
      flutter.debugPrint("${_color[level]!}${error.toString()}\x1B[0m"
          // _color[level]! + error.toString() + '\x1B[0m'
          );
    }
  }
}
