...
import 'side_bar_toggler.dart';
import 'side_navigation_bar_footer.dart';
import 'side_navigation_bar_header.dart';
import 'side_navigation_bar_item.dart';
import 'side_navigation_bar_theme.dart';
...
class SideNavigationBar extends StatefulWidget {
  ...
  final List<SideNavigationBarItem> footerItems;
...
class _SideNavigationBarState extends State<SideNavigationBar> {
  ...
  void _evaluateInitialStructure() {
    ...
    _minWidth = widget.minWidth != null ? widget.minWidth as double : _minWidth;
    _maxWidth = widget.maxWidth != null ? widget.maxWidth as double : _maxWidth;
  ...
  /// Generates [SideNavigationBarItemWidget] based on data provided in the interface
  /// [SideNavigationBarItem]
  List<Widget> _generateFooterItems() {
    List<Widget> _items = widget.footerItems
        .asMap()
        .entries
        .map<SideNavigationBarItemWidget>((MapEntry<int, SideNavigationBarItem> entry) => SideNavigationBarItemWidget(
              itemData: entry.value,
  ...
  Border? _evaluateMainDivider() {
    ...
    return Border(
      right: BorderSide(
        ...
        color: _theme.dividerTheme.mainDividerColor ?? (MediaQuery.of(context).platformBrightness == Brightness.light ? Colors.grey[700]! : Colors.white),
      ),
    );
  }
  ...
  Widget _evaluateFooterDivider() {
    ...
    if (widget.footer == null && widget.footerItems.isEmpty) {
      return Container();
    }

    if (!widget.expandable && widget.footerItems.isEmpty) {
      return Container();
    }
