...
class ToggleButtonsCustom extends StatelessWidget {
  ...
  const ToggleButtonsCustom({
    ...
    this.unselectedColor, // <--- newly added
    this.unselectedGradient, // <--- newly added
    this.disabledColor,
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> buttons =
      ...
      final bool isSelectedState = isSelected[index];
      final bool isUnselectedState = !isSelectedState;

      // == 1. Decide the foreground (text/icon) color ==
      // This is exactly as in your original code:
      final Color currentColor;
      if (!isDisabled && isSelected[index]) {
        // Selected & enabled
        currentColor = selectedColor ??
            toggleButtonsTheme.selectedColor ??
            theme.colorScheme.primary;
      } else if (!isDisabled && !isSelected[index]) {
        // Unselected & enabled
        currentColor = color ??
            toggleButtonsTheme.color ??
            theme.colorScheme.onSurface.withOpacity(0.87);
      ...
      // == 2. Decide which gradient to use (selected, unselected, disabled) ==
      // If you also have unselectedGradient, you can fold it in here.
      // For this snippet, we'll keep it simple:
      final Gradient? currentGradient = isDisabled && disabledGradient != null
          ? disabledGradient
          : (isSelectedState && selectedGradient != null
              ? selectedGradient
              : (isUnselectedState && unselectedGradient != null
                  ? unselectedGradient
                  : null));

      // == 3. Fallback fill color when no gradient is used ==
      // (You can replicate your unselectedColor logic here if needed.)
      final Color effectiveFillColor =
          _ResolveFillColor(fillColor ?? toggleButtonsTheme.fillColor).resolve({
                if (isSelectedState && !isDisabled) MaterialState.selected,
                if (isDisabled) MaterialState.disabled,
              }) ??
              _DefaultFillColor(theme.colorScheme).resolve({
                if (isSelectedState && !isDisabled) MaterialState.selected,
                if (isDisabled) MaterialState.disabled,
              });

      // -- or if you prefer a simpler approach:
      // final Color fallbackColor = isDisabled
      //   ? (disabledColor ?? theme.colorScheme.onSurface.withOpacity(0.38))
      //   : (isSelectedState
      //       ? (selectedColor ?? theme.colorScheme.primary)
      //       : (unselectedColor ?? color ?? theme.colorScheme.onSurface.withOpacity(0.87)));

      // == 4. The rest of your code is the same (borders, text styles, etc.) ==
      ...
      switch (tapTargetSize ?? theme.materialTapTargetSize) {
        case MaterialTapTargetSize.padded:
          if (direction == Axis.horizontal) {
            ...
            minPaddingSize = const Size(kMinInteractiveDimension, 0.0);
          }
          break;
        case MaterialTapTargetSize.shrinkWrap:
        default:
      ...
      Widget button = _SelectToggleButton(
        ...
        child: ClipRRect(
          borderRadius: clipBorderRadius,
          // Container for gradient or fallback color
          child: Container(
            decoration: BoxDecoration(
              gradient: currentGradient,
              // If no gradient, fill with `effectiveFillColor`
              color: currentGradient == null ? effectiveFillColor : null,
            ),
            child: TextButton(
              focusNode: focusNodes != null ? focusNodes![index] : null,
              style: ButtonStyle(
                // If gradient is used, background is transparent
                backgroundColor: currentGradient != null
                    ? const MaterialStatePropertyAll<Color>(Colors.transparent)
                    : MaterialStatePropertyAll<Color>(effectiveFillColor),
                // Symbol color (icon/text)
                foregroundColor: MaterialStatePropertyAll<Color>(currentColor),
                // Keep your overlay logic, shapes, etc.
                overlayColor: _ToggleButtonDefaultOverlay(
                  selected: !isDisabled && isSelectedState,
                  unselected: !isDisabled && !isSelectedState,
                  colorScheme: theme.colorScheme,
                  disabledColor:
              ...
              onPressed: isDisabled ? null : () => onPressed!(index),
              child: children[index],
            ),
          ),
        ),
