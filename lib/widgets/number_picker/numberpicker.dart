...
class NumberPicker extends StatefulWidget {
  ...
  /// Waits before center
  /// This is a workaround for a bug in the scrollController
  final Duration waitBeforeCenter;
  ...
  const NumberPicker({
    ...
    this.waitBeforeCenter = const Duration(milliseconds: 100),
...
class _NumberPickerState extends State<NumberPicker> {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: NotificationListener<ScrollEndNotification>(
        onNotification: (not) {
          _isUserInteracting = false; // Scrolling stopped, reset interaction status
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    // Determine the spacing adjustment based on how far the item is from the selected value
    double itemSpacing = widget.itemHeight; // Default item height
    int distanceFromSelected = (value - widget.value).abs();

    if (distanceFromSelected == 1) {
      itemSpacing = widget.itemHeight * 0.8; // Slightly smaller for +/-1
    } else if (distanceFromSelected == 2) {
      itemSpacing = widget.itemHeight * 0.6; // Even smaller for +/-2
    } else if (distanceFromSelected > 2) {
      itemSpacing = widget.itemHeight * 0.4; // Smaller for values further away
    }
    ...
    return Container(
      width: widget.itemWidth,
      height: itemSpacing, // Adjusted height based on distance
      alignment: Alignment.center,
      child: child,
    );
  }
  ...
  void _maybeCenterValue() {
    if (_scrollController.hasClients && !_isUserInteracting) {
      ...
      // Do not reset to index 0 unless absolutely necessary
      if (index != (_scrollController.offset / itemExtent).round()) {
